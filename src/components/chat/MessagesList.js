...
export default function MessagesList({
	...
	const getMessageStyle = (message, index, allMessages) => {
		...
		return {
			marginBottom: isGroupedWithNext ? '3px' : '8px',
							...
							>
								<div className="relative max-w-[75%] sm:max-w-[70%]">
									{messageStyle.showTimestamp && (
										<p className={`message-timestamp text-text-secondary -mt-1 opacity-75 mb-1 text-xs ${isOwn ? 'text-right mr-1' : 'text-left ml-1'
											}`}>
									...
									>
										{message.replyTo && (
											<div className={`mb-2 p-2 rounded-lg border-l-2 text-xs ${isOwn
												? 'bg-primary-light bg-opacity-30 border-white'
												: 'bg-gray-100 border-gray-300'
												}`}>
												<p className={`text-xs font-thin ${isOwn ? 'text-white opacity-60' : 'text-gray-600'}`}>{message.replyTo.sender?.username}</p>
												<p className={`text-xs truncate mt-0.5 ${isOwn ? 'text-white opacity-90' : 'text-gray-500'}`}>{message.replyTo.content}</p>
											</div>
										)}

										<div className={`flex items-end gap-2 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
											<div className="flex-1">
												<p className="message-content break-words select-text">{message.content}</p>
											</div>
												...
												<div className="flex gap-1 flex-shrink-0">
													{message.reactions.map((reaction, idx) => (
														<button
															...
															className={`inline-flex items-center px-1.5 py-0.5 text-xs rounded-full transition-all ${reaction.users.some(u => u._id === user?._id)
